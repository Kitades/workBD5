...
def load_employers():
    employers_ids = settings.get_employer_ids()
    sql = """
    ...
    try:
        with db_manager.connection.cursor() as cursor:
            for employer_id in tqdm(employers_ids, desc='Загрузка рабоьодателей'):
...
def load_vacancies():
    ...
    try:
        with db_manager.connection.cursor() as cursor:
            for employer_id in tqdm(employers_ids, desc='Загрузка вакансий'):
                vacancies = api_client.get_employer_vacancies(employer_id)
                data = (
                    (vac.id, vac.name, vac.url, vac.type.name, vac.salary_to, vac.salary_from, employer_id)
